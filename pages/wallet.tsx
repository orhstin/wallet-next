...
import { refreshBalance, handleAirdop } from "../utils";
...
const Wallet: NextPage = () => {
    ...
    useEffect(() => {
        ...
        refreshBalance(network, account).then((updatedBalance) => {
    ...
    const airdrop = async () => {
        ...
        const updatedBalance = await handleAirdop(network, account);
    ...
    const displayAddress = (address: string) => `${address.slice(0, 4)}...${address.slice(-4)}`;
                ...
                <Dashboard>
                    ...
                    <p>
                        ...
                        {network && 
                            (network === "mainnet-beta"
                             ? network.charAt(0).toUpperCase() + network.slice(1, 7) : network.charAt(0).toUpperCase() + network.slice(1, 7))}
                    </p>
                    {airdropLoading ? (
                        <h2>
                    ...
                    {network === "devnet" && account && (
                        <>
                            ...
                            <Tooltip
                                title="Click to receive 1 devnet SOL"
                                placement={"right"}>
                                <Question>?</Question>
                            </Tooltip>
                        </>
